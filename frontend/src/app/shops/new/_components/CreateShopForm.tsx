...
import type { QuotaStatus } from '@/lib/freemium'

type Props = {
    quotaStatus: QuotaStatus
}

export function CreateShopForm({ quotaStatus }: Props) {
    const [state, formAction, isPending] = useActionState(createShop, null)
    const router = useRouter()

    // QUOTA_EXCEEDED エラー時にモーダルを表示
    const isQuotaExceeded = state?.success === false && state.error === 'QUOTA_EXCEEDED'
    ...
    return (
        <>
            ...
            >
                {/* エラーバナー（QUOTA_EXCEEDED以外のエラーを表示） */}
                {state && !state.success && state.error !== 'QUOTA_EXCEEDED' && (
                ...
                {quotaStatus.plan === 'FREE' && (
                    <div className="flex items-center justify-between rounded-lg bg-amber-50 px-3 py-2 text-sm">
                        <span className="text-amber-700">今月のGoogle検索登録</span>
                        <span className="font-semibold text-amber-800">
                            残り {quotaStatus.remaining === Infinity ? '∞' : quotaStatus.remaining} 件 / {quotaStatus.baseLimit}件
                        </span>
                    </div>
                )}
            ...
            {/* クォータ上限モーダル（QUOTA_EXCEEDED エラー時に表示） */}
            <QuotaLimitModal
                open={isQuotaExceeded}
                canAddVideo={quotaStatus.canAddVideo}
                onVideoRewarded={() => router.refresh()}
            />
        </>
    )
}
